...
def test_create_user(email: str, public_users_client: PublicUsersClient):
    request = CreateUsersRequestSchema(email=fake.email(domain=email))
...
@pytest.mark.users
@pytest.mark.regression
def test_get_user_me(private_users_client: PrivateUsersClient, function_user: PublicUsersClient):
